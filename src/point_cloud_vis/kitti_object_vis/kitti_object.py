...
class kitti_object(object):
    ...
    def __init__(self, root_dir, label_dir, split="training", args=None):
        ...
        self.split_label_dir = os.path.join(label_dir, split)
        ...
        self.label_dir = os.path.join(self.split_label_dir, "label_2")
...
def show_lidar_with_depth(
    ...
    mlab.view(distance=85)
    mlab.show(1)

    return box3d_pts_3d_velo_list

def show_lidar_with_depth_custom(
    pc_velo,
    objects,
    calib,
    fig,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--dir",
        type=str,
        default="/media/drcl/DATADRIVE1/kitti",
        metavar="N",
...
def get_box3d_pts_list(objects, calib):
    """ Show all LiDAR points.
        Draw 3d box in LiDAR point cloud (in velo coord system) """
    if "mlab" not in sys.modules:
        import mayavi.mlab as mlab
    ...
    for obj in objects:
        if obj.type == "DontCare":
            continue
        # Draw 3d bounding box
        _, box3d_pts_3d = utils.compute_box_3d(obj, calib.P, only_3d_box = False)
        box3d_pts_3d_velo = calib.project_rect_to_velo(box3d_pts_3d)
