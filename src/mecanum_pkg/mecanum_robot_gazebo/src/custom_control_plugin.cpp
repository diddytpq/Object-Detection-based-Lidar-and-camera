...
/*
 * ver 2 Modified by : Yo Seph Yang
 * Date: 27 jun 2022
 */
...
namespace gazebo
{
    ...
    class CustomControlPlugin : public ModelPlugin
    {
...
namespace gazebo
{
    ...
    // Load the controller
    void CustomControlPlugin::Load(physics::ModelPtr parent,
            sdf::ElementPtr sdf)
    {
        ...
        {
            ROS_INFO_NAMED("Custom_Control_Plugin", "CustomControlPlugin missing <robotNamespace>, "
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <commandTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <odometryTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <odometryFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <robotBaseFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <odometryRate>, "
                    "defaults to %f",
        ...
        {
            ROS_WARN_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = %s) missing <cmdVelTimeout>, "
                    "defaults to %f",
        ...
        {
            ROS_FATAL_STREAM_NAMED("Custom_Control_Plugin", "CustomControlPlugin (ns = " << robot_namespace_
        ...
        vel_sub_ = nh_->subscribe<geometry_msgs::Twist>(command_topic_, 1,
                &CustomControlPlugin::cmdVelCallback, this);
        odometry_pub_ = nh_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

        // listen to the update event (broadcast every simulation iteration)
        update_connection_ =
            event::Events::ConnectWorldUpdateBegin(
                    boost::bind(&CustomControlPlugin::UpdateChild, this));

        ROS_INFO("Custom Control Plugin loaded");
    }

    // Update the controller
    void CustomControlPlugin::UpdateChild()
    {
    ...
    void CustomControlPlugin::FiniChild()
    {
        nh_->shutdown();
    }

    void CustomControlPlugin::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
    ...
    void CustomControlPlugin::publishOdometry(double step_time)
    {
    ...
    GZ_REGISTER_MODEL_PLUGIN(CustomControlPlugin)
}
